package com.lyy.stock.plugin.bgg.loadbalancer;
...
/**
 * @Author:
 * @createTime: 2023/03/23 18:08:01
 * @version:
 * @Description:
 */
public class DefaultLoadBalancer implements ReactorServiceInstanceLoadBalancer {
    ...
    private Response<ServiceInstance> getInstanceResponse(List<ServiceInstance> instances, HttpHeaders headers) {
        ...
        if (instances.isEmpty()) {
            ...
            String route = headers.getFirst(StockConstant.BGG_ROUTE);
    ...
    private Response<ServiceInstance> getServiceInstanceResponseByMetadataName(List<ServiceInstance> instances, String versionNo) {
        ...
        versionMap.put(StockConstant.VERSION, versionNo);
        //去除重复
    ...
    private Response<ServiceInstance> getServiceInstanceResponseWithWeight(List<ServiceInstance> instances) {
        ...
        for (ServiceInstance instance : instances) {
            ...
            Map<String, String> metadata = pluginAdapter.getServerMetadata(instance);
            //如果当前元信息包含权重key
            if (metadata.containsKey(StockConstant.NACOS_WEIGHT)) {
                weightList.add(new ImmutablePair<>(instance, Double.valueOf(metadata.get(StockConstant.NACOS_WEIGHT)).intValue()));
            }
        }
